...
#pragma endregion
		...
		else if (choice == 7) { // print list reverse

			printReverse(GetPTail(pList));
		}
...
Boolean insertContactInOrder(Node** pList, Contact newData) {
	...
	if (strcmp((*pList)->data.name, newData.name) > 0) {
		...
		newNode->pPrev = (*pList)->pPrev;
		(*pList)->pPrev = newNode;
		*pList = newNode;
		return TRUE;
	}
	else if (!(*pList)->pNext) { // next node is null
...
void printMenu() {
	...
	printf("\t7. print list reverse\n");
	printf("\t8. exit program\n");
}

Node* GetPTail(Node* pList) {

	if (pList == NULL) return NULL; // empty list

	if (pList->pNext == NULL) return pList; // end of list

	return GetPTail(pList->pNext); // recusively search
}

void printReverse(Node* pTail) {

	if (!pTail) return; // base case: null pointer

	// print current
	printContact(&pTail->data);
	putchar('\n');

	printReverse(pTail->pPrev); // recursive call
}